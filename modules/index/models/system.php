        ...
        if ($request->initSession() && $request->isSafe() && $login = Login::isMember()) {
            if (Login::checkPermission($login, 'can_config') && Login::notDemoMode($login)) {
                try {
                    ...
                    foreach (array('user_forgot', 'user_register', 'welcome_email', 'member_only', 'demo_mode', 'show_title_logo', 'activate_user', 'new_members_active', 'new_line_title') as $key) {
                        $config->$key = $request->post($key)->toBoolean();
                    }
                    foreach (array('login_fields') as $key) {
                        $value = $request->post($key, array())->filter('a-z0-9_');
                        if (empty($value)) {
                            $ret['ret_'.$key] = 'Please select at least one item';
                        } else {
                            $config->$key = $value;
                        }
                    }
                    ...
                    foreach (array('bg_color', 'color', 'warpper_bg_color', 'header_color', 'footer_color') as $key) {
