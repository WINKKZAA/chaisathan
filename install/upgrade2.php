    ...
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        if (!$error) {
            ...
            try {
                ...
                if (!$db->tableExists($table)) {
                    $sql = 'CREATE TABLE `$table` (';
                    $sql .= '`type` varchar(20) COLLATE utf8_unicode_ci NOT NULL,';
                    $sql .= ' `prefix` varchar(10) COLLATE utf8_unicode_ci NOT NULL DEFAULT "",';
                    $sql .= ' `auto_increment` int(11) NOT NULL,';
                    $sql .= ' `last_update` date NOT NULL';
                    $sql .= ') ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci ADD PRIMARY KEY (`type`,`prefix`)';
                    $db->query($sql);
                    $content[] = '<li class="correct">ปรับปรุงตาราง `'.$table.'` สำเร็จ</li>';
                }
                // ตาราง enroll_plan
